...
- **Only DBus/libvirt caller:** No route or other module imports `dbus-next` for libvirt or calls libvirt except through `vmManager`. Linux libvirt logic lives under `linux/vmManager/`; the `vmManager.js` facade re-exports the platform implementation. (Avahi uses `dbus-next` in `linux/mdnsManager.js` only.)
- **Domain object paths:** `/org/libvirt/domains/<uuid>`. UUIDs from ListDomains. Always obtain a fresh proxy — do not cache proxy references across reconnects.
