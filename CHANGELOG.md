...
- **VM/container detail sections read-only on phones** — Container Env and VM Network interfaces stack to no-scroll single-column read-only rows; VM Disks drops its Image type + Actions columns below `sm` (Disk/Size/Image/Bus remain); Container Mounts keeps Edit-file/Upload while hiding inline field editing and add controls. VM Network's duplicate "stop the VM to edit" lock badge is dropped (note below the table kept). USB rows show bare bus/device numbers instead of repeating the labels.
