...
### Bug Fixes
- **Caddy hosts editable on phones** — the wildcard domain shows once above the table instead of repeating in every row, the column is labeled Hostname, the hostname/target inputs get real widths with tighter gutters on phones, and the table is contained on desktop instead of stretching across the card.
...
- **VM/container action bar fits phones** — below `lg` the header actions become a full-width horizontally-scrollable row (primary actions leftmost, nothing dropped); the VM's redundant Overview tab strip is hidden and View XML is desktop-only; containers keep their Overview/Logs tabs.
- **VM/container detail sections read-only on phones** — Container Env stacks to no-scroll single-column read-only rows; VM Network interfaces stays one line per NIC with only the Actions column dropped below `sm`; VM Disks drops its Image type + Actions columns (Disk/Size/Image/Bus remain); Container Mounts keeps Edit-file/Upload while hiding inline field editing and add controls. VM Network's duplicate "stop the VM to edit" lock badge is dropped (note below the table kept). USB hides the ID column below `sm` and shows bare bus/device numbers instead of repeating the labels.
...
- **Clone now carries cloud-init artifacts.** The clone path filtered to `disk.device === 'disk'`, so the sde CDROM where `cloud-init.iso` lives was skipped — the cloned XML still pointed at the source VM's `cloud-init.iso`, and `cloud-init.json` (Wisp metadata, not in domain XML) was never copied. Filter now copies any disk/CDROM whose `<source file>` lives inside the source VM directory (cloud-init.iso among them) and remaps the XML; shared library ISOs stay shared by reference. The clone route copies `cloud-init.json` sibling-to-sibling. Cloned VMs carry the same cloud-init instance-id so first-boot setup does not re-run; editing cloud-init in the UI for the clone changes the user-data hash and forces a re-run on next boot if needed.
- **Backup modal no longer flips back to its initial state after a successful backup.** A parent-level `useEffect` in `OverviewPanel` (and the container equivalent) was clearing `backupJobId` the moment the job hit `status: 'done'`, which blanked `progress` mid-modal and reverted the dialog to its initial Cancel/Start state. The cleanup is now gated on the modal being closed, so the user sees "Backup complete." until they dismiss (or the new auto-close timer fires). The modal also auto-closes ~1.2s after the `done` event so the success state is acknowledged without a redundant click; errors still hold the modal open.
- **One privileged helper failing no longer skips every helper after it.** `helper.sh` ran under `set -e`, so a transient `pkg_install` error (commonly: dpkg lock from unattended-upgrades, exit 100) aborted before the script + sudoers entry could be installed; that cascaded into `install-helpers.sh` (also `set -e`) abandoning every subsequent block. `pkg_install` is now best-effort (the package is usually preinstalled by `packages.sh` anyway), per-helper failures are tracked, the script keeps going, and a final banner lists what failed and how to retry. Surfaces the same failure to `setup-server.sh`'s step tracker via a non-zero exit so it's not silent at any layer.
...
### Bug Fixes
- **Container `/etc/resolv.conf` choice no longer races a missing stub IP** — `resolveContainerResolvConf` and the post-CNI route install now decide by bridge name (`isVlanLikeBridgeName`) instead of probing the bridge's current addresses. The wisp DNS stub IP is asserted on every wisp start, so probing during a restart window could bake the wrong resolv.conf into the container's run; deciding by name removes the race. VLAN sub-bridges still fall through to host resolvers — Linux bridges don't route, so the stub is unreachable from a sub-bridge segment.
- **ISO attach, change, and eject now hot-plug on running VMs** — the backend already used `flags = isRunning ? 3 : 2` (LIVE+CONFIG vs CONFIG-only); the only blocker was a frontend gate hiding **+ CD-ROM** behind `isStopped`. Removed. CDROM rows also get a new **Change ISO** action that opens the library to swap media in one click.
- **Cloud-init edits now land on next boot, and qemu-guest-agent comes up reliably on first boot.** Four composing fixes in `lib/cloudInit.js`: instance-id derives from `sha256(user-data)[:12]` instead of constant `vmName` so edits aren't silently skipped by cloud-init's `once-per-instance` keying; `runcmd: systemctl enable --now qemu-guest-agent` runs after package install (the unit's `BindsTo=` virtio-port device window closes before mid-boot install completes, leaving the unit disabled); top-level `chpasswd` emitted alongside `users[].passwd` so password edits actually apply (`cc_users_groups` skips existing users); existing `cloud-init.iso` is unlinked before regen so libvirt's `dynamic_ownership` chown doesn't EACCES the truncate.
- **VM stats invalidate the guest-agent probe cache on `AgentEvent`** — the 30 s TTL on `InterfaceAddresses`/`GetHostname` previously kept the UI stuck on "disconnected" for up to 30 s after qemu-ga actually came online mid-boot. `vmManagerStats` now subscribes to `subscribeAgentEvent` and drops the cache on connect/disconnect; the next 5 s SSE tick re-probes fresh. TTL still earns its keep for DHCP renewals and in-guest hostnamectl changes (no event for those).
- **OS-update: concurrent apt calls coalesce, package list cached, external lock surfaced as 409.** Concurrent check/list now share a single in-flight `wisp-os-update` subprocess instead of failing one. Lock contention from `unattended-upgrades` (helper exit 75) surfaces as a soft "another op is running" warn rather than a red error. Hourly background check switched to the list op so the package cache is primed and "View packages" opens instantly.

### Refactor
- **Manager directory layout: `lib/{linux,darwin}/{vm,container}Manager` → `lib/{vm,container}Manager/{linux,darwin}`.** Pure rename, matches the layout of every other named module under `lib/` (`mdns/`, `networking/`, `storage/`, `host/`). `lib/vmManager.js` → `lib/vmManager/index.js`; `lib/containerManager.js` → `lib/containerManager/index.js`; `lib/vmManagerShared.js` → `lib/vmManager/vmManagerShared.js` (private to vmManager). The empty `lib/linux/` and `lib/darwin/` directories are gone. All call sites — backend boot, routes, manager-internal imports, doc references — updated in the same commit. Step 3 (final) of the managers-events-and-layout campaign.
- **Container mDNS event-driven via containerManager; managers carry zero `registerAddress` calls.** New `containerManager.subscribeContainerNetworkChange((name, snapshot) => ...)` — snapshot is `{ state, ip }` while running on a bridge, `null` otherwise — fires only on change. Linux-internal `containerManagerNetworkEvents.js` subscribes to the existing list-cache change events and runs a 60s netns probe; on IP diff it **persists the new IP into `container.json` before firing the event** (containerManager owns the durable record, fixes inverted ownership where the reconciler used to write a file owned by the manager). All seven prior manager → mdns A-record calls removed: `containerManagerList` refresh-on-read, `containerManagerConfig` localDns toggle, `containerManagerCreate` create + rollback, `containerManagerLifecycle` stop + kill, `containerManagerRename` cleanup. SRV/TXT service registration stays in containerManager. `containerMdnsReconciler.js` rewritten to subscribe-only (no own polling, no own writes). `routes/containers.js` calls `publishContainer` / `unpublishContainer` for metadata flips (localDns toggle, rename, pre-delete). Boot-time mDNS warm-up loop in `backend/src/index.js` deleted — replay-on-subscribe in both event surfaces handles it. Replay-on-subscribe also added to `subscribeVMNetworkChange` (new handlers fire once per current snapshot before subscribe returns). Step 2 of the managers-events-and-layout campaign.
- **VM mDNS publisher → reconciler, event-driven via vmManager.** New `vmManager.subscribeVMNetworkChange((name, snapshot) => ...)` on the public facade — snapshot is `{ stateCode, ip, hostname }` while running, `null` when stopped — fires only on actual change. Linux-internal `vmManagerNetwork.js` wires `DomainEvent` + `AgentEvent` + a 60s safety probe and auto-attaches per-domain agent listeners. `lib/vmMdnsPublisher.js` (and its `linux/`/`darwin/` impls) replaced by a single flat `lib/vmMdnsReconciler.js` that imports only the public vmManager + mdns facades — same shape as `containerMdnsReconciler.js`. Reconciler periodic cadence aligned at 60s for VMs and containers. `getGuestNetwork` exposed on the facade so glue can read current network state without touching libvirt internals.
- **`lib/downloads/` carved out** — `downloadFromUrl.js`, `downloadUtils.js`, `downloadArchCloud.js`, `downloadHaos.js`, `downloadUbuntuCloud.js`, and `fileTypes.js` move under `lib/downloads/` with a single `index.js` facade. Sole consumer (`routes/library.js`) imports through the facade. Step 6 commit 3 of the modules-boundaries refactor campaign.
- **`lib/jobs/` carved out** — `jobStore.js`, the five per-area job stores (`backupJobStore`, `containerJobStore`, `createJobStore`, `downloadJobStore`, `imageUpdateJobStore`), `listBackgroundJobs.js`, `backgroundJobKinds.js`, and `backgroundJobTitles.js` move under `lib/jobs/` with a single `index.js` facade. Routes import the namespace stores (`createJobStore`, `backupJobStore`, `downloadJobStore`) and the named exports through the facade. Step 6 commit 2 of the modules-boundaries refactor campaign.
- **Strict-managers boundary — vmManager + containerManager carry zero Wisp-glue imports.** Five edges broken in step 6 of the modules-boundaries campaign: (A1) `createAppError` inlined into `vmError`/`containerError` factories, dropping the `routeErrors.js` import from `vmManagerShared.js`, `containerManagerConnection.js`, darwin facade; (A2) validators vendored as `vmManagerValidation.js` + `containerValidation.js` (routes still use `lib/validation.js` unchanged); (A3) `vmManagerCloudInit.js` deleted from the manager tree — orchestration merged into `lib/cloudInit.js` (Wisp glue), and `vmManager.attachISO`/`ejectISO` generalized with `{ createIfMissing }`/`{ removeSlot }` opts so cloud-init's slot-create / slot-remove paths ride the public ISO primitives; create-time cloud-init build + rollback moved from `vmManagerCreate.js` to `routes/vms.js` POST handler; (A4) `publishVm`/`unpublishVm` calls moved from `vmManagerConfig.js` to `routes/vms.js` PATCH (publisher already subscribes to DomainEvent for everything else — the imperative call only existed because metadata-only changes don't fire one); (A5) `renameWorkloadAssignment` moved from `containerManagerRename.js` to `routes/containers.js` PATCH. New rule documented in `docs/WISP-RULES.md` § Architecture and pinned in root `CLAUDE.md`: future changes to vmManager / containerManager are appropriate only for **generic libvirt/containerd functionality**; Wisp-specific orchestration goes in routes or app-glue. Other carved modules (`host/`, `networking/`, `mdns/`, `storage/`) remain looser. Step 6 of the modules-boundaries refactor campaign.
...
### Bug Fixes
- **Container `.local` resolution broke after creating/deleting a managed VLAN bridge** — `wisp-bridge` runs `netplan apply` which re-renders bridges from declared YAML, dropping the runtime-only `169.254.53.53/32` from br0. Containers' DNS queries to the in-process forwarder then black-holed. Fix: re-assert the stub IP from `wisp-bridge`'s `apply_netplan()` after every `netplan apply`. (Initial attempt declared the stub in netplan YAML alongside `dhcp4: true` — that triggered a systemd-networkd reconcile loop on Ubuntu that flapped the LAN address and broke mDNS host publishing entirely; reverted. See `docs/plans/setup-refactor.md` for the full lesson.) Pre-existing fragility, surfaced while testing the modules-boundaries step 1 refactor.
...
- **Storage module carved out** — new `lib/storage/` (facade `index.js`, cross-platform `diskOps.js`, `linux/{diskMonitor,diskMount,smbMount,smart}.js`, `darwin/{diskMonitor,diskMount,smbMount,smart}.js`). Replaces the flat `diskOps.js`/`smbMount.js`/`diskMount.js`/`diskMonitor.js` facades plus their `linux/host/` impls and `linux/host/diskSmart.js`. SMART summaries lifted into storage (was at `linux/host/`); `hostHardware.js` becomes a consumer of `storage.readAllDiskSmartSummaries` instead of importing `diskSmart` as a sibling. `mountsAutoMount.js` (the boot reconcile + hotplug glue) stays at `lib/` top-level as Wisp app-level glue. Step 3 of the modules-boundaries refactor campaign.
- **Host introspection module carved out** — new `lib/host/` (facade `index.js`, `linux/{hostHardware,hostGpus,hostPower,usbMonitor,procStats,rebootRequired,osUpdates,smart,pciIds}.js`, `darwin/` parallels including `systemProfiler{Hardware,Software}.js`). Replaces the seven flat top-level facades (`hostHardware.js`, `hostGpus.js`, `hostPower.js`, `usbMonitor.js`, `procStats.js`, `rebootRequired.js`, `aptUpdates.js` — the last renamed to drop the `apt`-ism since impls are already platform-neutral `osUpdates.js`) plus their `linux/host/` and `darwin/host/` impls. `pciIds.js` moves into the module as an internal helper. SMART (`readDiskSmartSummary`/`readAllDiskSmartSummaries`) moves from `storage/` to `host/` — sole consumer is `hostHardware.js`, and neither manager will ever need it (revises step 3's placement). `linuxProcUptime.js` moves into `linux/containerManager/procUptime.js` since containerManager is its sole consumer; resolves the only manager→host deep import. Step 4 of the modules-boundaries refactor campaign.
- **Dropped `vmManager.listHostUSBDevices()` relay** — the function was a one-line pass-through to `host.getDevices()`, exported from both Linux and macOS vmManager facets. The only consumer (`GET /api/host/usb`) now calls the host facade directly. Eliminates vmManager → host coupling entirely; the only manager → host import left is `containerManager/apps/jellyfin.js` calling `listHostGpus()` (real GPU-passthrough policy, not a relay).
- **containerApps carved out of containerManager** — new cross-platform `lib/containerApps/` (registry, four app modules, `createAppContainer`/`applyAppConfig`/`eject` glue + `maskAppSecrets`). containerManager loses ~160 lines of app-aware code from create + config update; gains a generic opaque `metadata` field on `container.json` plus pass-through handling for `services`/`runAsRoot`/`env`/`mounts`/`devices` at create. App identity now lives at `container.json.metadata.app` (string id) + `metadata.appConfig` (object) — manager never introspects either. App orchestration (validate → derive → call manager primitives → write mount file contents → live-reload-or-pendingRestart) lives entirely in `containerApps`, which is a peer consumer of `containerManager` (same shape as routes). UI reads from `config.metadata?.app` / `config.metadata?.appConfig`. Pre-refactor app containers will surface as generic containers after upgrade — feature-building mode, recreate to restore app UI. Step 4b of the modules-boundaries refactor campaign.
