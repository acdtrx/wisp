...
- `.local` resolution survives host reboots — `wisp-backend.service` re-asserts `169.254.53.53/32` on `br0` and per-link `MulticastDNS=resolve` on every boot (both are runtime-only and otherwise dropped at reboot)
