...
MDNS_DIR="/var/lib/wisp/mdns"
SMB_MOUNT_DIR="/mnt/wisp/smb"
mkdir -p "$IMAGE_DIR" "$VMS_DIR" "$BACKUPS_DIR" "$CONTAINERS_DIR" "$MDNS_DIR" "$SMB_MOUNT_DIR"
chown "$DEPLOY_USER:libvirt" "$IMAGE_DIR" "$VMS_DIR" "$BACKUPS_DIR" "$CONTAINERS_DIR" "$MDNS_DIR" "$SMB_MOUNT_DIR"
chmod 0775 "$IMAGE_DIR" "$VMS_DIR" "$BACKUPS_DIR" "$CONTAINERS_DIR" "$MDNS_DIR" "$SMB_MOUNT_DIR"
...
echo "  $MDNS_DIR: created, owned by $DEPLOY_USER:libvirt"
