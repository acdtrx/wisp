...
mkdir -p "$IMAGE_DIR" "$VMS_DIR" "$BACKUPS_DIR" "$CONTAINERS_DIR" "$WISP_MOUNT_ROOT"
chown "$DEPLOY_USER:libvirt" "$IMAGE_DIR" "$VMS_DIR" "$BACKUPS_DIR" "$CONTAINERS_DIR" "$WISP_MOUNT_ROOT"
chmod 0775 "$IMAGE_DIR" "$VMS_DIR" "$BACKUPS_DIR" "$CONTAINERS_DIR" "$WISP_MOUNT_ROOT"
