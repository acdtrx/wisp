...
- **Persistence:** `169.254.53.53/32` is a runtime-only address on br0 — it's intentionally NOT declared in netplan (declaring it alongside `dhcp4: true` triggers a systemd-networkd reconcile loop on Ubuntu's bridge handling that flaps the LAN address and breaks mDNS publishing; see `docs/plans/setup-refactor.md`). It's asserted at three points: at install time by `container-dns.sh`, at every wisp service start by `wisp.service` `ExecStartPre=+` (privileged exec via `+`, even though the service runs as the deploy user), and after every `netplan apply` from `wisp-bridge` (managed VLAN bridge create/delete). The unit declares `After=network-online.target Wants=network-online.target` so `br0` exists when `ExecStartPre` runs.
