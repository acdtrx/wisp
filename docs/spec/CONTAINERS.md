...
- **Persistence:** `169.254.53.53/32` is declared in `/etc/netplan/90-wisp-bridge.yaml` under `bridges.br0.addresses`, so `netplan apply` (boot, manual, or invoked by `wisp-bridge` during managed VLAN bridge create/delete) preserves it as part of br0's declared state. `bridge.sh` writes the stub IP for new installs; `bridge-config.sh` patches existing installs' YAML in place (called by `setup-server.sh` and `wisp-updater`). Defense-in-depth: `wisp.service` also carries an `ExecStartPre=+` line (privileged exec via `+`, even though the service runs as the deploy user) that re-asserts the IP idempotently at every service start; `container-dns.sh` does the same at install time. The unit declares `After=network-online.target Wants=network-online.target` so `br0` exists by then.
