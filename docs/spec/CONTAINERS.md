...
- **Network probe + IP persistence:** `linux/containerManager/containerManagerNetworkEvents.js` runs every **60 s** and, for every running bridge container, reads the live netns IP via `discoverIpv4InNetnsOnce`. If the live IP differs from `network.ip` in `container.json`, containerManager **atomically rewrites the config first**, then fires `subscribeContainerNetworkChange` so the reconciler refreshes the A record. Without this loop, an asynchronous DHCP renewal would leave the published `.local` host pointing at the stale IP forever — and other readers of `getContainerConfig` would see stale state.
