...
- **Persistence:** `169.254.53.53/32` is **declared to systemd-networkd** via a drop-in at `/etc/systemd/network/<br0-unit>.d/wisp-mdns-stub.conf` (`[Network] Address=169.254.53.53/32`), written by `container-dns.sh` at install time and re-written by `wisp-updater` on every self-update. The script resolves `<br0-unit>` from `networkctl status br0`'s `Network File:` line rather than hardcoding a path, so it covers both layouts Wisp produces — netplan's `/run/systemd/network/10-netplan-br0.network` on Ubuntu and `bridge.sh`'s `/etc/systemd/network/90-wisp-br0.network` on Arch. Drop-ins live under `/etc` even when the unit itself is in `/run`. networkd therefore re-applies the address on every restart and `networkctl reconfigure`.
  - **Why it is declared and not runtime-only:** as a runtime address it was flushed by any networkd restart — an unattended systemd upgrade is enough. The failure is silent and total: the forwarder's socket stays bound to the now-nonexistent address (`ss` still reports it listening), so **every** container DNS query black-holes, not just `.local`, until the wisp process restarts. Observed 2026-07-31 as a 10-hour outage surfacing only as Caddy 502s.
  - **Why a networkd drop-in and not netplan:** an `addresses:` entry alongside `dhcp4: true` on a bridge puts networkd into a reconcile loop that flaps the LAN address and breaks mDNS publishing — tried and rolled back, see `docs/plans/setup-refactor.md`. The drop-in bypasses netplan's bridge handler and settles at `routable (configured)`.
  - **One runtime assertion remains**, for hosts networkd does not manage (NetworkManager) and for the boot window before br0 is configured: `wisp.service` `ExecStartPre=+` at every service start (privileged exec via `+`, even though the service runs as the deploy user). The unit declares `After=network-online.target Wants=network-online.target` so `br0` exists when `ExecStartPre` runs. `wisp-bridge` no longer re-asserts the address after its `netplan apply` — networkd re-applies the drop-in as part of the re-render, so that workaround is gone rather than stacked on top of the fix.
  - **Known gap:** a host where netplan renders to **NetworkManager** takes `bridge.sh`'s netplan branch (so `wisp-bridge` works) but has no networkd-managed br0, so no drop-in is written and the address stays runtime-only — a VLAN bridge create/delete drops it until the next wisp restart. Wisp targets networkd-rendered netplan (Ubuntu Server default) and Arch's systemd-networkd; `container-dns.sh` prints which mode it took. The declarative equivalent there is `nmcli connection modify <con> +ipv4.addresses 169.254.53.53/32` (NetworkManager persists it in the connection profile and re-applies it on activation) — not implemented, since Wisp has no NetworkManager host to verify it on.
