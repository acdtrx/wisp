...
1. **General** — one row: Name, **DNS** toggle (Local DNS — advertise `<name>.local` via mDNS; help icon), then (divider) CPU Cores, RAM (number + GB/MB toggle), then (divider) OS Type (segmented: Linux/Windows, flexes to fill), **Auto Start** toggle at the right end. Per-section dirty state and Save button.
...
1. **General** — mirrors the VM General layout. First row: Name (editable when stopped), **DNS** toggle (Local DNS — advertise `<name>.local` via mDNS; help icon), then (divider) **CPU**, **RAM (MiB)** (compact VM-style number fields, no unit switch — empty = unlimited `∞`), then (divider) Image (flexes to fill, with library picker button). Second row: **Command Override** (flexes to fill), **Run as Root**, **Restart Policy** (segmented control), then (divider) **Auto Start**, **Auto Backup** (`Toggle` + help icon — opts the container into the daily scheduled backup, see Host Mgmt → Backup Scheduler) at the right end. In create mode only Name and Image render.
