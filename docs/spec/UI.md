...
2. **Environment Variables** — **Table** (DataTable chrome); header **`Plus`+braces** adds a row; **view** / **Edit** / row **Save**/**Cancel**; remove icon-only; **no section Save**. Secret rows show a **`Dices`** action: opens a confirm dialog with a client-generated random value (32-byte hex) shown once in a copyable "New secret" field; **Apply** fills the row (edit mode, dirty) and the row **Save** commits — old value unrecoverable after save. Persistence: [UI-PATTERNS.md](../UI-PATTERNS.md) (**Container environment** — full **`env`** map on row save, documented exception).
