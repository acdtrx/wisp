...
3. **Mounts** — Section header: **Plus + File** / **Plus + Folder** (add mount). **Table:** type icon, mount name, container path, read-only toggle, actions. Mutations use **row-scoped** container mount APIs (add / update / delete / upload); see [UI-PATTERNS.md](../UI-PATTERNS.md) and [CONTAINERS.md](CONTAINERS.md). **Hover-reveal** actions when not editing; per-row edit/save/cancel where applicable.
