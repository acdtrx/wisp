...
1. **General** — Name (read-only), image, command override, **CPU Cores**, **Memory (MiB)**, then (after a divider): **Run as Root**, **Restart Policy** (segmented control ~18rem wide — between the ~15.5rem compact and ~26rem wide bars), **Auto Start** (same `Toggle` as VM Advanced), **Auto Backup** (`Toggle` + help icon — opts the container into the daily scheduled backup, see Host Mgmt → Backup Scheduler). Labels align on one row with CPU/RAM.
