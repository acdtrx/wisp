...
- **VM/container detail sections are read-only on phones** with inline editing hidden below `sm`. Two tiers: (a) **fully stacked, no scroll** — Container Env (value under key; actions hidden) and VM Network interfaces (bridge/model/MAC/VLAN stacked under `net<n>`; actions hidden); (b) **read-only + kept actions, table still scrolls** for the widest/multi-row sections — VM Disks (size/bus editing hidden, but Eject/Change ISO and Detach kept) and Container Mounts (field editing + delete hidden, but Edit-file/Upload kept). All add controls are hidden below `sm`. Full add/edit returns via the deferred modal-editor redesign (`docs/BACKLOG.md`).
