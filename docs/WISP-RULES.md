...
- Only `backend/src/lib/linux/vmManager/` (and the `vmManager.js` facade) imports `dbus-next` for **libvirt** or calls libvirt. Avahi/mDNS uses `dbus-next` only in `lib/mdns/linux/avahi.js` (separate DBus service). New VM operations = new vmManager function.
