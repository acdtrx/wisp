...
- **vmManager and containerManager are strict.** They must not import any Wisp app glue — explicitly: `routeErrors.js`, `validation.js`, `cloudInit.js`, `vmMdnsReconciler.js`, `containerMdnsReconciler.js`, `sections.js`, `paths.js`, `config.js`, `settings.js`, `atomicJson.js`, `loadRuntimeEnv.js`, `bootCleanup.js`, `mountsAutoMount.js`, anything under `lib/jobs/`, `lib/downloads/`, or any route helper. They may import only stdlib, their own internal files (including `vmManagerShared.js`), and the carved cross-cutting modules `mdns/`, `networking/`, `storage/`. **Future changes to vmManager or containerManager are appropriate only for generic libvirt/containerd functionality** — Wisp-specific orchestration (cloud-init flow, section assignments, mDNS publish hooks, file-cleanup rollbacks) goes in routes or app-glue files, which call into the managers via their public surface. The other carved modules (`host/`, `networking/`, `mdns/`, `storage/`) are looser — they may import `createAppError` from `routeErrors.js` and other small Wisp helpers.
- **App-level glue stays at `lib/` top-level.** Files that wire two reusable modules together (e.g. `vmMdnsReconciler`, `containerMdnsReconciler`, `cloudInit.js`) are *integration code* — Wisp's specific policy for how vmManager events drive mDNS side-effects, when to reconcile, what hostname to advertise, how cloud-init artifacts are built and persisted. They live flat at `backend/src/lib/`, **not** inside `vmManager/` or `containerManager/` or `mdns/`. This keeps each named module independently extractable as a standalone lib; consumers in other projects write their own glue against the same event surfaces. Shared infrastructure used by both modules and glue (e.g. `paths.js`, `validation.js`) also stays flat at `lib/` until carved into a named module by a later refactor step.
- **Manager event surfaces (push, not pull).** Both managers expose subscribe-and-replay events for network state on the public facade:
  - `vmManager.subscribeVMNetworkChange((name, snapshot) => {...})` — `snapshot = { stateCode, ip, hostname }` while running, `null` when stopped. Linux impl wires DomainEvent + AgentEvent + a 60s safety probe internally.
  - `containerManager.subscribeContainerNetworkChange((name, snapshot) => {...})` — `snapshot = { state, ip }` while running on a bridge, `null` otherwise. Linux impl wires the containerd events stream + a 60s netns probe internally, **and persists the new IP into `container.json` before firing the event** (containerManager owns the durable record).
  - Both fire only on change against the last emitted snapshot, replay current state to new subscribers, and are no-ops on darwin.
  - Glue (`vmMdnsReconciler`, `containerMdnsReconciler`) subscribes via the public facade and stays platform-agnostic — neither piece of glue reaches into libvirt or netns helpers. All A-record register/deregister calls live in the reconcilers; the managers carry **zero** `registerAddress` calls. SRV/TXT services stay in containerManager (tied to the per-container services array) — a follow-up cleanup.
  - New event surfaces follow the same pattern: emit only on change, replay-on-subscribe, expose a single `subscribe…` function on the facade, keep platform plumbing private.
- Only `backend/src/lib/vmManager/linux/` (and the `vmManager.js` facade) imports `dbus-next` for **libvirt** or calls libvirt. Avahi/mDNS uses `dbus-next` only in `lib/mdns/linux/avahi.js` (separate DBus service). New VM operations = new vmManager function.
...
- **Single containerd caller**: Only `backend/src/lib/containerManager/linux/` imports `@grpc/grpc-js`. Routes and other libs must not import grpc-js directly.
