...
- **Manager event surfaces (push, not pull).** `vmManager.subscribeVMNetworkChange((name, snapshot) => {...})` fires when a VM's `(stateCode, ip, hostname)` differs from the last emitted state for that VM. snapshot is `null` when the VM stops. The Linux impl wires DomainEvent + AgentEvent + a 60s safety probe internally so consumers don't reach into libvirt. Glue (`vmMdnsReconciler`) subscribes via the public facade and stays platform-agnostic. New event surfaces follow the same pattern: emit only on change, expose a single `subscribe…` function on the facade, keep platform plumbing private to the manager.
