...
- **Single DBus/libvirt caller:** Only `backend/src/lib/vmManager/linux/` (and the `vmManager.js` facade) may import `dbus-next` for **libvirt** or talk to libvirt. Avahi/mDNS uses `dbus-next` only in `lib/mdns/linux/avahi.js`. Routes and other libs must not import `dbus-next` for libvirt. New VM operations = new purpose-named export from the vmManager facade.
- **Single gRPC/containerd caller:** Only `backend/src/lib/containerManager/linux/` (and the `containerManager.js` facade) may import `@grpc/grpc-js` or talk to containerd. Routes and other libs must not import `@grpc/grpc-js`. New operations = new purpose-named export from the containerManager facade.
- **Strict managers:** vmManager and containerManager carry **zero Wisp-glue imports**. They may import only stdlib, their own internal files (incl. `vmManagerShared.js`), and the cross-cutting modules `mdns/`, `networking/`, `storage/`. They must never reach into `routeErrors`, `validation`, `cloudInit`, `vmMdnsReconciler`, `sections`, `paths`, `config`, `settings`, `atomicJson`, `bootCleanup`, jobs, downloads, or any route helper. Future changes to vmManager / containerManager are appropriate only for **generic libvirt/containerd functionality**; Wisp-specific orchestration goes in routes or app-glue. Other carved modules (`host/`, `networking/`, `mdns/`, `storage/`) are looser. Full rule + rationale in `docs/WISP-RULES.md` § Architecture.
