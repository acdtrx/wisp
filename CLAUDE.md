...
- **Strict managers:** vmManager and containerManager carry **zero Wisp-glue imports**. They may import only stdlib, their own internal files (incl. `vmManagerShared.js`), and the cross-cutting modules `mdns/`, `networking/`, `storage/`. They must never reach into `routeErrors`, `validation`, `cloudInit`, `vmMdnsPublisher`, `sections`, `paths`, `config`, `settings`, `atomicJson`, `bootCleanup`, jobs, downloads, or any route helper. Future changes to vmManager / containerManager are appropriate only for **generic libvirt/containerd functionality**; Wisp-specific orchestration goes in routes or app-glue. Other carved modules (`host/`, `networking/`, `mdns/`, `storage/`) are looser. Full rule + rationale in `docs/WISP-RULES.md` § Architecture.
